...
function Employers() {
	...
	const { loading, error, data } = useQuery(queries.GET_EMPLOYERS_WITH_EMPLOYEES, { fetchPolicy: 'cache-and-network' });
	...
	if (data) {
		...
		return (
			<div>
				...
				{/*Add Employer Modal */}
				{showAddModal && <AddModal isOpen={showAddModal} handleClose={handleCloseModals} modal='addEmployer' />}
			</div>
		);
	} else if (loading) {
