...
(function (global) {
  ...
  extend(QUnit, {
    ...
    module: function (name, testEnvironment, executeNow) {
      ...
      if (
        testEnvironment &&
        (testEnvironment.setup || testEnvironment.teardown)
      ) {
        console.warn(
      ...
      function createModule() {
        var parentModule = config.moduleStack.length
          ...
          : null;
        var moduleName =
          parentModule !== null ? [parentModule.name, name].join(' > ') : name;
        var module = {
          name: moduleName,
    ...
    hooks: function (handler) {
      ...
      function processHooks(test, module) {
        ...
        if (
          ...
          hooks.push(
            test.queueHook(module.testEnvironment[handler], handler, module)
          );
        }
      }
  ...
  QUnit.assert = Assert.prototype = {
    ...
    pushResult: function (resultInfo) {
      ...
      var assert = this,
        currentTest =
          (assert instanceof Assert && assert.test) || QUnit.config.current;
    ...
    throws: function (block, expected, message) {
      var actual,
        ...
        ok = false,
        currentTest =
          (this instanceof Assert && this.test) || QUnit.config.current;
...
(function () {
  ...
  function applyUrlParams() {
    var i,
      selectedModules = [],
      modulesList = id('qunit-modulefilter-dropdown-list').getElementsByTagName(
        'input'
      ),
      ...
      while (pointer < diffs.length - 1) {
        if (
          ...
          if (position === diffs[pointer - 1][1]) {
            ...
            diffPointer.substring(0, diffs[pointer + 1][1].length) ===
            diffs[pointer + 1][1]
          ) {
