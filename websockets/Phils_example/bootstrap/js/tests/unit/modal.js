$(function () {
  ...
  QUnit.test('should expose defaults var for settings', function (assert) {
    assert.expect(1);
    assert.ok(
      $.fn.bootstrapModal.Constructor.Default,
  ...
  QUnit.test(
    'should remove aria-hidden attribute when shown, add it back when hidden',
    function (assert) {
      assert.expect(3);
      var done = assert.async();
      ...
      $('<div id="modal-test" aria-hidden="true"/>')
        .on('shown.bs.modal', function () {
          assert.notOk(
            $('#modal-test').is('[aria-hidden]'),
            'aria-hidden attribute removed'
          );
          $(this).bootstrapModal('hide');
        })
        .on('hidden.bs.modal', function () {
          ...
          assert.strictEqual(
            $('#modal-test').attr('aria-hidden'),
            'true',
            'correct aria-hidden="true" added'
          );
          done();
        })
        .bootstrapModal('show');
    ...
    function (assert) {
      ...
      $(
        ...
        .on('hidden.bs.modal', function () {
          setTimeout(function () {
            assert.ok(
              $(document.activeElement).is($toggleBtn),
    ...
    function (assert) {
      ...
      $(
        ...
        .on('hidden.bs.modal', function () {
          setTimeout(function () {
            assert.ok(
              $(document.activeElement).is($otherBtn),
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      var done = assert.async();
      var $body = $(document.body);
      var $style = $('<style>body { padding-right: 42px; }</style>').appendTo(
        'head'
      );
