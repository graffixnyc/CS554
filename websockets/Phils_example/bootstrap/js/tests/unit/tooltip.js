$(function () {
  ...
  QUnit.test('should place tooltips relative to placement option', function (
    ...
    assert.ok(
      $('.tooltip').is(
        '.fade.bs-tether-element-attached-top.bs-tether-element-attached-center.in'
      ),
  ...
  QUnit.test('should destroy tooltip', function (assert) {
    ...
    assert.ok(
      $._data($tooltip[0], 'events').mouseover &&
        $._data($tooltip[0], 'events').mouseout,
      'tooltip has hover events'
    );
    assert.strictEqual(
      $._data($tooltip[0], 'events').click[0].namespace,
      'foo',
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      var top = Math.round(
        $target.offset().top +
          $target[0].offsetHeight / 2 -
          $tooltip[0].offsetHeight / 2
      );
  ...
  QUnit.test(
    'should not error when trying to show an top-placed tooltip that has been removed from the dom',
    function (assert) {
      assert.expect(1);
      var passed = true;
      var $tooltip = $('<a href="#" rel="tooltip" title="Another tooltip"/>')
  ...
  QUnit.test('should wait 200ms before hiding the tooltip', function (assert) {
    ...
    setTimeout(function () {
      assert.ok(
        $($tooltip.data('bs.tooltip').tip).is('.fade.in'),
      ...
      setTimeout(function () {
        assert.ok(
          !$($tooltip.data('bs.tooltip').tip).is('.in'),
          '200ms: tooltip removed'
        );
        done();
      }, 200);
    }, 0);
  ...
  QUnit.test('should correctly position tooltips on SVG elements', function (
    assert
  ) {
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      var $element = $('<div id="target" title="1"/>').appendTo(
        '#qunit-fixture'
      );
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      function showingTooltip() {
        return $tooltip.hasClass('in') || tooltip._hoverState == 'in';
      }
