$(function () {
  ...
  QUnit.test(
    'should allow DOMElement title and content (html: true)',
    function (assert) {
      assert.expect(5);
      ...
      assert.strictEqual(
        $('.popover .popover-title').text(),
        '@glebm <3 writing tests',
        'title inserted'
      );
      assert.ok(
        $.contains($('.popover').get(0), title),
        'title node moved, not copied'
      );
      // toLowerCase because IE8 will return <I>...</I>
      assert.strictEqual(
        $('.popover .popover-content').html().toLowerCase(),
        '<i>¯\\_(ツ)_/¯</i>',
        'content inserted'
      );
      assert.ok(
        $.contains($('.popover').get(0), content),
  ...
  QUnit.test(
    'should allow DOMElement title and content (html: false)',
    function (assert) {
      assert.expect(5);
      var title = document.createTextNode('@glebm <3 writing tests');
      var content = $('<i>¯\\_(ツ)_/¯</i>').get(0);
  ...
  QUnit.test('should destroy popover', function (assert) {
    ...
    assert.ok(
      $._data($popover[0], 'events').mouseover &&
        $._data($popover[0], 'events').mouseout,
      'popover has hover event'
    );
    assert.strictEqual(
      $._data($popover[0], 'events').click[0].namespace,
      'foo',
